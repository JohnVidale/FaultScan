...
def find_long_trace_file(
    data_path: Path,
    code_str: str,
    channel: str,
    sampling_hz: int,
) -> Path | None:
    """Resolve one long-trace input file across naming variants."""
    chan_dir = data_path / code_str / f"{channel}.D"
    if not chan_dir.exists():
        return None

    candidates = sorted(chan_dir.glob(f"7V.{code_str}.00.{channel}.D.2022.273*.mseed"))
    if not candidates:
        return None

    # Prefer the new uniform names without downXX suffix.
    preferred: list[Path] = [p for p in candidates if ".down" not in p.name]
    # Backward-compatible fallback for older naming with down50/down100.
    if not preferred and sampling_hz in (50, 100):
        preferred = [p for p in candidates if f"down{sampling_hz}" in p.name]

    return preferred[0] if preferred else candidates[0]


def find_snippet_file(data_path: Path, eve_id: str, sta_code3: str, channel: str) -> Path | None:
    """Resolve one snippet file using the new unified naming and fallback globs."""
    chan_dir = data_path / sta_code3 / f"{channel}.D"
    if not chan_dir.exists():
        return None

    expected = chan_dir / f"{eve_id}_{sta_code3}_{channel}.mseed"
    if expected.exists():
        return expected

    fallback = sorted(chan_dir.glob("*.mseed"))
    if not fallback:
        return None
    return fallback[0]
...
def read_waveforms_for_event(
    ...
    for sta, (slat, slon) in name2ll.items():
        ...
        for ch_read in chan_list_this_sta:
            if input_is_snippets:
                fpath = find_snippet_file(
                    data_path=data_path,
                    eve_id=eve_id,
                    sta_code3=code_3,
                    channel=ch_read,
